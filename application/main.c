...
static T_PsdkReturnCode ReceiveXPortSystemState(T_PsdkGimbalSystemState systemState)
{
    printf("receive XPort system state: mounted upward flag %d, gimbal mode %d.",
                            systemState.mountedUpward, systemState.gimbalMode);

    printf("XPort fine tune: %d %d %d.", systemState.fineTuneAngle.pitch,
...
{
    printf("receive XPort attitude information:\n");
    printf("XPort attitude: pitch %d, roll %d, yaw %d.\n", attitudeInformation.attitude.pitch,
...
int main(void)
{
    ...
    PsdkLogger_UserLogInfo("Returned to main!\n");
    
    //------------------------------------------------------------Added for data subscription storage----------------------------------------------------//
    ...
    FILE* fp = fopen("data_subscription_data_in_main.txt", "a+");
    ...
    while (1) {
        ...
        if (psdkStat != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            ...
            fprintf(fp, "velocity: x %f y %f z %f, healthFlag %d.\n", velocity.data.x, velocity.data.y,
                                    velocity.data.z, velocity.health);
        }
