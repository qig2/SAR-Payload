...
#define PSDK_MEDIA_FILE_PATH            "../../../../../api_sample/camera_media_emu/media_file/" //media file path
...
#define VIDEO_FILE_PATH                 "../../../../../api_sample/camera_media_emu/media_file/moop.mp4" //video file to play
...
// Created export function by us
void video_stream_in_main(void) {
	/*
	int ret;
    T_PsdkReturnCode psdkStat;
   ...
   */
	...
	PsdkLogger_UserLogInfo("Test!\n");
	// Create a video thread
	if (PsdkOsal_TaskCreate(&s_userSendVideoThread, UserCameraMedia_SendVideoTask, "user_camera_media_task", 2048, NULL) != PSDK_RETURN_CODE_OK) {
		PsdkLogger_UserLogError("user send video task create error.");
		// return PSDK_RETURN_CODE_ERR_UNKNOWN;
	}	
	PsdkLogger_UserLogInfo("Thread created!\n");
...
PsdkPlayback_VideoFileTranscode(const char *inPath, const char *outFormat, char *outPath, uint16_t outPathBufferSize)
{
	...
	printf("inPath=%s    directory = %s aabbcc\n", inPath, directory);
